...
- (void)queryForAllPostsNearLocation:(PFGeoPoint *)point withNearbyDistance:(CLLocationAccuracy)nearbyDistance;
...
- (void)locationDidChange:(NSNotification *)note {
	AppDelegate *appDelegate = [[UIApplication sharedApplication] delegate];
    //appDelegate.filterDistance = [[[note userInfo] objectForKey:kPAWFilterDistanceKey] doubleValue];
    ...
    CLLocationCoordinate2D coord = appDelegate.currentLocation.coordinate;
    PFGeoPoint *point = [PFGeoPoint geoPointWithLatitude:coord.latitude longitude:coord.longitude];
	[self queryForAllPostsNearLocation:point withNearbyDistance:appDelegate.filterDistance];
...
- (void)postWasCreated:(NSNotification *)note {
	AppDelegate *appDelegate = [[UIApplication sharedApplication] delegate];
    PFObject *post = note.object;
    PFGeoPoint *point = [post objectForKey:kPAWParseLocationKey];
    
    // FIX THIS LINE. IT'S ANNOYING AS HELL
    // THEN GET LOCATION TO DISPLAY ON MINI MAP
    // THEN CHANGE THE HEIGHT OF THE MESSAGE BOX
    // HEIGHT OF MESSAGE BOX MAYBE BE ABLE TO BE CHANGED. JUST NEED TO DIG THROUGH FILE
    // fyi: application does not work with an internet connection
	// [self queryForAllPostsNearLocation:point withNearbyDistance:appDelegate.filterDistance];
}
...
- (IBAction)settingsButtonSelected:(id)sender {
    ...
    UIViewController *settingsViewController = [sb instantiateViewControllerWithIdentifier:@"settings"];
	settingsViewController.modalTransitionStyle = UIModalTransitionStyleFlipHorizontal;
	[self.navigationController presentViewController:settingsViewController animated:YES completion:nil];
}

- (IBAction)postButtonSelected:(id)sender {
	//WallPostsTableViewController *createPostViewController = [[WallPostsTableViewController alloc] initWithNibName:nil bundle:nil];
    UIStoryboard*  sb = [UIStoryboard storyboardWithName:@"Main" bundle:nil];
    UIViewController *createPostViewController = [sb instantiateViewControllerWithIdentifier:@"post"];
...
- (void)locationManager:(CLLocationManager *)manager didChangeAuthorizationStatus:(CLAuthorizationStatus)status {
	...
	switch (status) {
        ...
        {{
            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"SkateSpot can’t access your current location.\n\nTo view nearby posts or create a post at your current location, turn on access for SkateSpot to your location in the Settings app under Location Services." message:nil delegate:self cancelButtonTitle:nil otherButtonTitles:@"Ok", nil];
            [alertView show];
	...
	[query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error) {
		if (error) {
			...
			for (PFObject *object in objects) {
				Post *newPost = [[Post alloc] initWithPFObject:object];
				[allNewPosts addObject:newPost];
				BOOL found = NO;
				for (Post *currentPost in allPosts) {
			...
			for (Post *newPost in newPosts) {
				...
				CLLocationDistance distanceFromCurrent = [location distanceFromLocation:objectLocation];
